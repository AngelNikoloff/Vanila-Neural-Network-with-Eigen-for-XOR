...
public:
	...
	Eigen::ArrayXd derivativ_h = Eigen::ArrayXd::Zero(hidden_size);
	Eigen::ArrayXd delta_err_h = Eigen::ArrayXd::Zero(hidden_size);
	Eigen::ArrayXd gradient_h = Eigen::ArrayXd::Zero(hidden_size);
	//==========================================================================
	SimpleXorNet_Eigen()
	{
	...
	void backpropogate()
	{
		...
		derivativ_h = 1 - h_v*h_v;
		delta_err_h = (m_o_W.transpose() * gradient.matrix()).array();
		gradient_h = derivativ_h * delta_err_h;
		//===============================
		d_h_W = gradient_h.matrix() * i_v.matrix().transpose();
		d_h_b = gradient_h.matrix() * m_h_b.matrix().transpose();
		//==================================================================== Update weights;
